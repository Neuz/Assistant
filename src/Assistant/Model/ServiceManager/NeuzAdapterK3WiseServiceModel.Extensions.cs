...
public static class NeuzAdapterK3WiseServiceModelExtensions
{
    ...
    public static async Task<NeuzAdapterK3WiseServiceModel> Flush(this NeuzAdapterK3WiseServiceModel model)
    {
        if (File.Exists(model.InsConfFilePath))
            model = await FileUtils.ReadFromConf<NeuzAdapterK3WiseServiceModel>(model.InsConfFilePath) ?? new NeuzAdapterK3WiseServiceModel();
        else
            model = new NeuzAdapterK3WiseServiceModel();
    ...
    /// <returns></returns>
    public static async Task<bool> Install(this NeuzAdapterK3WiseServiceModel model, string? zipFilePath)
    {
        try
        {
    ...
    /// <param name="model"></param>
    /// <returns></returns>
    public static async Task<bool> UnInstall(this NeuzAdapterK3WiseServiceModel model)
    {
        ArgumentNullException.ThrowIfNull(model.ServiceName, nameof(model.ServiceName));
        ArgumentNullException.ThrowIfNull(model.LogDirectory, nameof(model.LogDirectory));
        try
